...
mod tests {
    ...
    generate_addr_of_methods! {
        impl<> Entry {
            unsafe fn addr_of_pointers(self: NonNull<Self>) -> NonNull<Pointers<Entry>> {
                &self.pointers
            }
        }
    }
    ...
    type EntryList = LinkedList<Entry, <Entry as Link>::Target>;
    ...
    fn test_push_front() {
        ...
        let e = unsafe { list.pop_back().unwrap().as_mut() };
        assert_eq!(e.val, 1);
    }
}
